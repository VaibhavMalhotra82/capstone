...
def _infer_retrieval_intent(query: str, intent: str | None) -> str:
    ...
    if re.search(r"\b(contact|support|branch|hours?|location)\b", lowered):
        return "general_faq"
    return "general_faq"

@tool
@traceable(run_type="retriever")
async def advisory_engine(query: str, intent: str = "loan_inquiry") -> dict:
    ...
    try:
        ...
        result_dict = result.dict() if hasattr(result, "dict") else {"advice": str(result)}
