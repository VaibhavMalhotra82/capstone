...
async def process_query(user_query: str, session_id: str, callbacks=None) -> dict:
    ...
    try:
        ...
        should_prefetch_advisory = _needs_loan_advisory(user_query, intent_result["intent"]) and _needs_sip(user_query)

        if should_prefetch_advisory:
            advisory_query = _build_advisory_query(user_query)
            logger.log_info("Pre-fetching advisory guidance for mixed loan-and-savings query: %s" % (advisory_query,))
